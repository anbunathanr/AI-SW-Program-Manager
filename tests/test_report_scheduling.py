...
def mock_env(monkeypatch):
    ...
    monkeypatch.setenv(
        "REPORT_GENERATION_LAMBDA_ARN",
        "arn:aws:lambda:us-east-1:123456789012:function:report-gen",
    )
    monkeypatch.setenv(
        "EMAIL_DISTRIBUTION_LAMBDA_ARN",
        "arn:aws:lambda:us-east-1:123456789012:function:email-dist",
    )
...
def schedule_request_event():
    ...
    return {
        ...
        "requestContext": {
            "authorizer": {"tenantId": "tenant-123", "userId": "user-456"}
        },
    }
...
class TestScheduleReportHandler:
    ...
    def test_create_schedule_success(
        self, mock_dynamodb, mock_events, schedule_request_event, mock_context
    ):
    ...
    @patch("src.report_scheduling.handler.events_client")
    @patch("src.report_scheduling.handler.dynamodb")
    def test_create_schedule_missing_tenant_id(
        self, mock_dynamodb, mock_events, schedule_request_event, mock_context
    ):
    ...
    @patch("src.report_scheduling.handler.events_client")
    @patch("src.report_scheduling.handler.dynamodb")
    def test_create_schedule_invalid_report_type(
        self, mock_dynamodb, mock_events, schedule_request_event, mock_context
    ):
    ...
    @patch("src.report_scheduling.handler.events_client")
    @patch("src.report_scheduling.handler.dynamodb")
    def test_create_schedule_empty_recipients(
        self, mock_dynamodb, mock_events, schedule_request_event, mock_context
    ):
    ...
    @patch("src.report_scheduling.handler.events_client")
    @patch("src.report_scheduling.handler.dynamodb")
    def test_create_schedule_invalid_schedule_expression(
        self, mock_dynamodb, mock_events, schedule_request_event, mock_context
    ):
...
class TestDeleteScheduleHandler:
    ...
    def test_delete_schedule_success(self, mock_dynamodb, mock_events, mock_context):
        ...
        # Mock EventBridge
        mock_events.list_targets_by_rule.return_value = {"Targets": [{"Id": "1"}]}
...
class TestScheduleValidation:
    ...
    def test_validate_cron_expression(self):
        ...
        assert (
            _validate_schedule_expression("cron(invalid)") is True
        )  # EventBridge will validate syntax
    ...
    def test_validate_rate_expression(self):
        ...
        # Invalid expressions
        assert (
            _validate_schedule_expression("rate(invalid)") is True
        )  # EventBridge will validate syntax
...
class TestScheduledExecutionHandler:
    ...
    def test_scheduled_execution_success(
        self, mock_dynamodb, mock_lambda, mock_context
    ):
        """Test successful scheduled report execution."""
        from src.report_scheduling.scheduled_execution_handler import (
            scheduled_execution_handler,
        )
    ...
    def test_scheduled_execution_missing_tenant_id(self, mock_lambda, mock_context):
        ...
        event = {"report_type": "WEEKLY_STATUS", "recipients": ["user1@example.com"]}

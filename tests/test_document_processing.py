...
def s3_event_record():
    ...
    return {
        "s3": {
            ...
            "object": {"key": "tenant-123/documents/doc-456/test.pdf"},
        }
    }
...
def test_process_document_pdf_success(
    ...
    assert (
        call_args[1]["Document"]["S3Object"]["Name"]
        == "tenant-123/documents/doc-456/test.pdf"
    )
...
def test_process_document_txt_success(
    mock_sns, mock_dynamodb, mock_s3, mock_context, document_metadata
):
...
@patch("src.document_processing.handler.dynamodb")
@patch("src.document_processing.handler.sns_client")
def test_process_document_failure_notification(
    mock_sns,
    mock_dynamodb,
    mock_textract,
    mock_s3,

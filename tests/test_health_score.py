...
class TestHealthScoreComposition:
    ...
    **Validates: Requirements 18.1**
    ...
    def test_health_score_is_weighted_composite(
        self, velocity_score, backlog_score, milestone_score, risk_score
    ):
        """
        ...
        with patch(
            "health_score.score_calculator.calculate_velocity_score",
            return_value=velocity_score,
        ), patch(
            "health_score.score_calculator.calculate_backlog_score",
            return_value=backlog_score,
        ), patch(
            "health_score.score_calculator.calculate_milestone_score",
            return_value=milestone_score,
        ), patch(
            "health_score.score_calculator.calculate_risk_score",
            ...
            assert (
                abs(result["health_score"] - round(expected_score)) <= 1
    ...
    def test_custom_weights_applied_correctly(
        self, velocity_weight, backlog_weight, milestone_weight, risk_weight
    ):
        """
        ...
        with patch(
            "health_score.score_calculator.calculate_velocity_score",
            return_value=component_scores["velocity"],
        ), patch(
            "health_score.score_calculator.calculate_backlog_score",
            return_value=component_scores["backlog"],
        ), patch(
            "health_score.score_calculator.calculate_milestone_score",
            return_value=component_scores["milestones"],
        ), patch(
            "health_score.score_calculator.calculate_risk_score",
            return_value=component_scores["risks"],
        ):

            result = calculate_health_score(
                "project-1", "tenant-1", custom_weights=custom_weights
            )
...
class TestHealthScoreRange:
    ...
    **Validates: Requirements 18.2**
    ...
    @settings(max_examples=20, deadline=None)
    @given(component_score=st.floats(min_value=0, max_value=100))
...
class TestHealthScoreUpdateTriggering:
    ...
    **Validates: Requirements 18.3**
    ...
    @settings(max_examples=20, deadline=None)
    @given(project_id=st.uuids().map(str), tenant_id=st.uuids().map(str))
    def test_data_refresh_triggers_recalculation(self, project_id, tenant_id):
        ...
        with patch(
            "health_score.event_trigger.calculate_health_score"
        ) as mock_calc, patch(
            "health_score.event_trigger.store_health_score_history"
        ) as mock_store, patch(
            "health_score.event_trigger.publish_health_score_event"
        ) as mock_publish:
...
class TestHealthScoreHistoryPersistence:
    ...
    **Validates: Requirements 18.4**
    ...
    def test_health_score_stored_with_timestamp(
        self, project_id, tenant_id, health_score
    ):
        """
        ...
        with patch("health_score.score_storage.execute_query") as mock_query:
            ...
            assert (
                health_score_data["calculated_at"] in params
    ...
    def test_health_score_history_retrievable(
        self, project_id, tenant_id, history_count
    ):
        """
        ...
        with patch("health_score.score_storage.execute_query") as mock_query:
            ...
            history = get_health_score_history(
                project_id=project_id, tenant_id=tenant_id, limit=30
            )
...
class TestHealthScoreHandler:
    ...
    def test_calculate_health_score_handler_success(self):
        ...
        event = {"project_id": "project-123", "tenant_id": "tenant-456"}
        ...
        with patch("health_score.handler.calculate_health_score") as mock_calc, patch(
            "health_score.handler.store_health_score_history"
        ) as mock_store, patch(
            "health_score.handler.publish_health_score_event"
        ) as mock_publish:

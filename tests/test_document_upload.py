...
def test_upload_document_success(
    mock_dynamodb, mock_s3, mock_context, mock_authorizer_context, valid_upload_request
):
    ...
    event = {
        ...
        "requestContext": {"authorizer": mock_authorizer_context},
    }
    ...
    assert (
        "660e8400-e29b-41d4-a716-446655440001/documents/"
        in call_args[1]["Params"]["Key"]
    )
...
def test_upload_document_invalid_format(
    mock_dynamodb, mock_s3, mock_context, mock_authorizer_context
):
...
@patch("src.document_upload.handler.s3_client")
@patch("src.document_upload.handler.dynamodb")
def test_upload_document_invalid_document_type(
    mock_dynamodb, mock_s3, mock_context, mock_authorizer_context
):
...
def test_upload_document_missing_authorization(mock_dynamodb, mock_s3, mock_context):
    ...
    event = {"body": json.dumps(request), "requestContext": {}}
...
@patch("src.document_upload.handler.s3_client")
@patch("src.document_upload.handler.dynamodb")
def test_upload_document_tenant_specific_prefix(
    mock_dynamodb, mock_s3, mock_context, mock_authorizer_context, valid_upload_request
):

...
class IngestionWorkflowStack(Stack):
    ...
    def _create_sqs_queues(self) -> None:
        ...
        self.ingestion_queue = sqs.Queue(
            ...
            visibility_timeout=Duration.minutes(
                35
            ),  # Longer than state machine timeout (30 min)
    ...
    def _create_lambda_functions(self) -> None:
        ...
        self.fetch_jira_lambda = lambda_.Function(
            ...
            environment={"INTEGRATIONS_TABLE_NAME": self.integrations_table_name},
    ...
    def _create_state_machine(self) -> None:
        ...
        parallel_fetch = sfn.Parallel(
            self, "ParallelFetch", result_path="$.fetchResults"
        )
        ...
        validate_data_task = tasks.LambdaInvoke(
            ...
            payload=sfn.TaskInput.from_object({"fetchResults.$": "$.fetchResults"}),
            output_path="$.Payload",
            retry_on_service_exceptions=True,
        )
        ...
        definition = (
            parallel_fetch.next(validate_data_task)
        ...
        self.queue_processor_lambda.add_event_source(
            lambda_event_sources.SqsEventSource(
                ...
                max_batching_window=Duration.seconds(
                    10
                ),  # Wait up to 10 seconds to batch messages
...
"""
        ...
        self.scheduled_rule.add_target(
            targets.LambdaFunction(self.scheduled_trigger_lambda, retry_attempts=2)
        )
...
"""
        ...
        sync_resource.add_method(
            "POST", apigw.LambdaIntegration(self.manual_trigger_lambda, proxy=True)
        )
    ...
    def _create_dlq_alarms(self) -> None:
        ...
        # Add SNS action to alarm
        self.dlq_age_alarm.add_alarm_action(cw_actions.SnsAction(self.dlq_alarm_topic))

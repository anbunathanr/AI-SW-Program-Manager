...
class DatabaseMaintenanceStack(Stack):
    ...
    def __init__(
        self, scope: Construct, construct_id: str, database_stack, **kwargs
    ) -> None:
        ...
        self.refresh_views_rule.add_target(
            targets.LambdaFunction(
                ...
                event=events.RuleTargetInput.from_object({"task": "refresh_views"}),
            )
        )
        ...
        self.vacuum_analyze_rule.add_target(
            targets.LambdaFunction(
                self.maintenance_function,
                event=events.RuleTargetInput.from_object({"task": "vacuum_analyze"}),
            )
        )
        ...
        self.performance_check_rule.add_target(
            targets.LambdaFunction(
                self.maintenance_function,
                event=events.RuleTargetInput.from_object({"task": "check_performance"}),
            )
        )
        ...
        self.slow_refresh_alarm.add_alarm_action(cw_actions.SnsAction(self.alarm_topic))
        ...
        self.slow_query_alarm.add_alarm_action(cw_actions.SnsAction(self.alarm_topic))

        # Alarm: Lambda function errors
        self.maintenance_error_alarm = cloudwatch.Alarm(
            self,
            "MaintenanceFunctionErrors",
            metric=self.maintenance_function.metric_errors(period=Duration.minutes(5)),
            threshold=1,
        ...
        self.dashboard.add_widgets(
            ...
            cloudwatch.GraphWidget(
                ...
                left_y_axis=cloudwatch.YAxisProps(label="Milliseconds", min=0),
            ),
        )
        ...
        self.dashboard.add_widgets(
            ...
            cloudwatch.GraphWidget(
                ...
                left=[
                    self.maintenance_function.metric_errors(period=Duration.minutes(5))
                ],
